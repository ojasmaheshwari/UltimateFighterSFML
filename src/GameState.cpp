...
GameState::GameState(sf::RenderWindow *window, Game *game)
  ...
  m_Fighter1.setTextureRect(
      sf::IntRect(fighter1TextureSize.x * 0, fighter1TextureSize.y * 0,
                  fighter1TextureSize.x, fighter1TextureSize.y));
  // m_Fighter1.setPosition(22, m_Window->getSize().y - 66);
  m_Fighter1.setPosition(0, m_Window->getSize().y - m_ArenaGround.getSize().y -
                                m_Fighter1.getLocalBounds().getSize().y + 10);
  m_Logger.info(std::format("Global bounds are {}x{}, local bounds are {}x{}",
                            m_Fighter1.getGlobalBounds().getSize().x,
                            m_Fighter1.getGlobalBounds().getSize().y,
                            m_Fighter1.getLocalBounds().getSize().x,
                            m_Fighter1.getLocalBounds().getSize().y));
