  ...
  :m_Window(window), m_Game(game), m_Background((sf::Vector2f)m_Window->getSize()), m_Logger(LoggingLevel::LogLevelInfo, "MainMenuState"), m_MenuHeading("Main Menu", m_MenuFont, 100)
{
  ...
  if (!m_MenuFont.loadFromFile("assets/menu_font.otf")) {
  ...
  for (uint32_t i=0; i<m_ChoiceCount; i++) {
    m_MenuChoices[i].setFont(m_MenuFont);
  ...
  m_MenuChoices[1].setString("Help");
  ...
  if(!m_MenuChoiceChangeSoundBuffer.loadFromFile("assets/menu_choice_change.mp3")) {
